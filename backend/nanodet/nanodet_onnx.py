...
class NanoDetONNX(object):
    ...
    def _get_bboxes_single(
        ...
        if indexes is not None and len(indexes) > 0:
            ...
            # - list of ints: [0, 2, 5]
            # - list of single-item lists: [[0], [2], [5]]
            # - numpy array of shape (N, 1)
            # Normalize to 1D numpy array of indices
            indexes_np = np.array(indexes).reshape(-1).astype(np.int32)

            bboxes = bboxes[indexes_np]
            scores = scores[indexes_np]
            class_ids = class_ids[indexes_np]
        else:
            bboxes = np.array([])
            scores = np.array([])
            class_ids = np.array([])
