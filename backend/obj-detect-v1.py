...
START_STOP = 24
RUN_BRAKE = 12
CW_CCW = 14        # 元のコードの正しいPIN番号
INT_VR_EXT = 1     # 元のコードの正しいPIN番号
...
class MotorController:
    ...
    def initialize(self):
        ...
        GPIO.output(INT_VR_EXT, OFF)  # 最初はOFF
        time.sleep(2)  # 診断テストと同じ待機時間
        
        # ステップ2: INT.VR/EXT入力をON（内部速度設定モード）
        print("  ステップ2: INT.VR/EXT入力をON（内部速度設定モード）")
        GPIO.output(INT_VR_EXT, ON)
        time.sleep(1)  # 診断テストと同じ待機時間
        ...
        # 初期化後の信号状態を表示
        self._print_signal_status()
    
    def _print_signal_status(self):
        """現在の信号状態を表示"""
        print("\n--- 信号状態確認 ---")
        print(f"START_STOP (GPIO{START_STOP}): {'ON' if GPIO.input(START_STOP) == ON else 'OFF'}")
        print(f"RUN_BRAKE  (GPIO{RUN_BRAKE}):  {'ON' if GPIO.input(RUN_BRAKE) == ON else 'OFF'}")
        print(f"CW_CCW     (GPIO{CW_CCW}):     {'CW' if GPIO.input(CW_CCW) == ON else 'CCW'}")
        print(f"INT_VR_EXT (GPIO{INT_VR_EXT}): {'INT' if GPIO.input(INT_VR_EXT) == ON else 'EXT'}")
        print("-------------------\n")
    
    def start_cw(self):
        """CW方向（時計回り）で起動（診断テストと同じシーケンス）"""
        ...
        # ステップ2: START/STOP ON
        print("  ステップ2: START/STOP信号をON")
        GPIO.output(START_STOP, ON)
        time.sleep(1)  # 診断テストと同じ待機時間
        
        # ステップ3: RUN/BRAKE ON（運転開始）
        print("  ステップ3: RUN/BRAKE信号をON（運転開始）")
...
def main():
    ...
    print("=" * 60)
    print("物体検出統合型モーター制御システム（修正版）")
    print("=" * 60)
    print("修正内容:")
    print("  - 初期化シーケンスを診断テストと同じタイミングに変更")
    print("  - 起動時の待機時間を1秒に延長")
    print("  - INT_VR_EXTを最初にOFF→ONの順で設定")
    print("=" * 60)
    print()
